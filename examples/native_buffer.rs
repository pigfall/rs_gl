...
fn main(){
    ...
    let native_buffer = NativeBufferBuilder::from_vertex_buffer(&VertexBuffer::from_bytes(data_bytes,vec![VertexAttributeDescriptor{
    shader_location:0,
    num_of_demision:3,
    data_type: VertexAttributeDataType::F32,
    normalized:false,
    }]), NativeBufferBufferDataUsage::STATIC_DRAW).build(&mut state);
